...
public:
    ...
    string *operator->()const;
    str_blob_ptr &operator+(const size_t &);
...
str_blob_ptr &str_blob_ptr::operator+(const size_t &sz)
{
    check(curr,"increment past end of str_blob_ptr");
    curr += sz;
    check(curr,"increment past end of str_blob_ptr");
...
{
private:
    shared_ptr<str_blob_ptr> wpt;
public:
    str_blob_ptr_ptr() = default;
    str_blob_ptr_ptr(const str_blob_ptr &p):wpt(make_shared<str_blob_ptr> (p)){}
    str_blob_ptr *operator->()
    {
        return &(*wpt);
    }
...
int main()
{
    str_blob sb({"hello","world","kkk"});
    str_blob_ptr sbt(sb);
    str_blob_ptr_ptr sbtt(sbt);
    cout << sbtt->deref() << endl;
    return 0;
}
