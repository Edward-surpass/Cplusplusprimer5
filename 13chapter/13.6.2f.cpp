...
void message::move_folders(message *me)
{
    folders = me->folders;
    for(auto &e:folders)
    {
...
message::message(message &&me):conntents (std::move(me.conntents))
{
    move_folders(&me);
}
...
message &message::operator=(message &&me)
{
    ...
    {
        this->remove_from_folders();
        conntents = std::move(me.conntents);
        move_folders(&me);
    }
    return *this;
}
