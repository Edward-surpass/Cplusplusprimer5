...
str_vec::str_vec(str_vec &&vc)noexcept
{
    elements = vc.elements;
    first_free = vc.first_free;
    cap = vc.cap;
    vc.elements = vc.cap = vc.first_free = nullptr;
    vc.free();
}
...
str_vec &str_vec::operator=(str_vec &&vc)noexcept
{
    if(this != &vc)
    {
        elements = vc.elements;
